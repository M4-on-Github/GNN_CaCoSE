...
4 & SAGPool readout function & mean $\|$ max for NC; \textbf{sum} for GC & Paper says only ``READOUT''. Measured on MUTAG: sum gives $82.5$ vs $69.0$ for mean$\|$max, and macro-F1 $79.1$ vs $60.7$. Sum is the only injective multiset readout & \textbf{Resolved} (GC) \\
...
12 & GC evaluation: single 80/10/10 split, or $k$-fold? & Both provided; 80/10/10 is the default & The reported MUTAG accuracy $76.99$ is unreachable under the stated split: with 188 graphs the test set holds 18--20 graphs, so a 10-seed mean must be a multiple of $1/(10n)$, and no $n$ in that range admits $76.99$. $k$-fold has unequal fold sizes and does & Open \\
\bottomrule
\end{tabular}

\vspace{0.4em}
{\small Ambiguity log. Items 2, 4 and 7 are settled; the rest are working assumptions.
